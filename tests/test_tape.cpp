...
constexpr TapeParams::State TS_STOPPING = (
    TapeParams::State::TAPE_STATE_STOPPING
);
constexpr TapeParams::State TS_STOPPED = TapeParams::State::TAPE_STATE_STOPPED;
constexpr TapeParams::State TS_STARTABLE = (
    TapeParams::State::TAPE_STATE_STARTABLE
);
constexpr TapeParams::State TS_STARTING = (
    TapeParams::State::TAPE_STATE_STARTING
);
constexpr TapeParams::State TS_STARTED = TapeParams::State::TAPE_STATE_STARTED;
constexpr TapeParams::State TS_FF_STARTABLE = (
    TapeParams::State::TAPE_STATE_FF_STARTABLE
);
constexpr TapeParams::State TS_FF_STARTING = (
    TapeParams::State::TAPE_STATE_FF_STARTING
);
constexpr TapeParams::State TS_FF_STARTED = (
...
constexpr TapeStopTestStepSoundExpectation EXP_SILENCE = true;
constexpr TapeStopTestStepSoundExpectation EXP_SOUND = false;

constexpr TapeStopTestStepParamRampingExpectation EXP_RAMPING = true;
constexpr TapeStopTestStepParamRampingExpectation EXP_CONST = false;


Math::RNG rng(123);


TEST(tape_is_only_engaged_when_bypass_toggle_value_is_matched, {
    ...
    Tape<FixedSignalProducer, ToggleParam::OFF> tape_off(
        "F", params, input, rng
    );
    ...
    rendered = SignalProducer::produce<
        Tape<FixedSignalProducer, ToggleParam::OFF>
    >(
    ...
    rendered = SignalProducer::produce<
        Tape<FixedSignalProducer, ToggleParam::ON>
    >(
...
class TapeStopTestStep
{
    public:
        TapeStopTestStep(
                ...
                TapeStopTestStepParamRampingExpectation const
                    volume_param_expectation,
                TapeStopTestStepParamRampingExpectation const
                    delay_time_param_expectation,
...
void test_tape_stop(std::array<TapeStopTestStep, step_count> const& steps)
{
    ...
    for (i = 0; i != step_count; ++i) {
        ...
        Sample const* const* rendered = SignalProducer::produce<
            Tape<FixedSignalProducer, ToggleParam::ON>
        >(
            tape, (Integer)i, step.samples_to_render
        );

        if (step.output_expectation == EXP_SILENCE) {
        ...
        if (step.volume_param_expectation == EXP_RAMPING) {
            assert_true(
                tape_params.volume.is_ramping(), "step=%u", (unsigned int)i
            );
        } else {
            assert_false(
                tape_params.volume.is_ramping(), "step=%u", (unsigned int)i
        ...
        if (step.delay_time_param_expectation == EXP_RAMPING) {
            assert_true(
                tape_params.delay_time_lfo.min.is_ramping(),
                "step=%u",
                (unsigned int)i
            );
            assert_true(
                tape_params.delay_time_lfo.max.is_ramping(),
                "step=%u",
                (unsigned int)i
            );
        } else {
        ...
        assert_eq(
            step.expected_state, tape_params.state, "step=%u", (unsigned int)i
        );
    }
}
...
TEST(stopping_time_must_be_set_to_zero_before_first_use, {
    test_tape_stop<5>(
        {
            TapeStopTestStep(1.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_INIT),
            TapeStopTestStep(2.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_INIT),
            TapeStopTestStep(3.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_INIT),
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ),
            TapeStopTestStep(
                5.1, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_STOPPING
            ),
        }
    );
})
    ...
    test_tape_stop<6>(
        {
            ...
            TapeStopTestStep(
                6.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_STOPPING
            ),
            TapeStopTestStep(
                9.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_STOPPING
            ),
            TapeStopTestStep(
                9.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_STOPPING
            ),
            TapeStopTestStep(
                9.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPING
            ),
            TapeStopTestStep(
                9.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPED
            ),
        }
    );
})
    ...
    test_tape_stop<5>(
        {
            ...
            TapeStopTestStep(
                5.0, 1, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_FF_STARTING
            ),
            TapeStopTestStep(
                3.0, 1, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            TapeStopTestStep(
                2.0, 1, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTED
            ),
        }
    );
})
    ...
    test_tape_stop<6>(
        {
            ...
            TapeStopTestStep(
                4.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_STOPPING
            ),
            TapeStopTestStep(
                4.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPING
            ),
            TapeStopTestStep(
                4.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPED
            ),
            TapeStopTestStep(
                3.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPED
            ),
            TapeStopTestStep(
                2.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STOPPED
            ),
        }
    );
})
...
TEST(set_zero_start_stop_time_then_turn_it_up_to_fast_forward_stopped_tape, {
    test_tape_stop<8>(
        {
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ...
            TapeStopTestStep(
                0.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_FF_STARTABLE
            ),
            TapeStopTestStep(
                0.0, 5, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_FF_STARTABLE
            ),
            TapeStopTestStep(
                5.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            TapeStopTestStep(
                5.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_FF_STARTING
            ),
        }
    );
})
    ...
    test_tape_stop<8>(
        {
            ...
            TapeStopTestStep(
                5.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STARTABLE
            ),
            TapeStopTestStep(
                5.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_STARTING
            ),
            TapeStopTestStep(
                5.0, 1, EXP_SOUND, EXP_CONST, EXP_CONST, TS_STARTING
            ),
            TapeStopTestStep(
                5.0, 1, EXP_SOUND, EXP_CONST, EXP_CONST, TS_STARTED
            ),
        }
    );
})
...
TEST(stopped_and_quickly_started_tape_waits_for_a_zero_stop_time, {
    test_tape_stop<12>(
        {
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ...
            TapeStopTestStep(
                6.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_STARTED
            ),
            TapeStopTestStep(
                3.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_STARTED
            ),
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ),
...
TEST(fast_forwarding_tape_schedules_new_ffwd_when_stop_start_time_is_changed, {
    test_tape_stop<11>(
        {
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ...
            TapeStopTestStep(
                2.0, 1, EXP_SILENCE, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            TapeStopTestStep(
                5.0, 5, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_FF_STARTING
            ),
            TapeStopTestStep(
                5.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            TapeStopTestStep(
                3.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            TapeStopTestStep(
                3.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTED
            ),
        }
    );
})
...
TEST(fast_forwarded_tape_waits_for_zero_start_stop_time, {
    test_tape_stop<12>(
        {
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ...
            TapeStopTestStep(
                3.0, 3, EXP_SOUND, EXP_RAMPING, EXP_RAMPING, TS_FF_STARTING
            ),
            TapeStopTestStep(
                3.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTING
            ),
            ...
            TapeStopTestStep(
                2.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTED
            ),
            TapeStopTestStep(
                5.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_FF_STARTED
            ),
            TapeStopTestStep(
                0.0, 5, EXP_SOUND, EXP_CONST, EXP_CONST, TS_NORMAL
            ),
